...
class ClientController {
    ...
    public function index() {
        ...
        $campaignRepo = new MysqlCampaignRepository();
        $campaignsList = $campaignRepo->findAllActive();
    ...
    public function showEdit() {
        ...
        $campaignRepo = new MysqlCampaignRepository();
        $campaignsList = $campaignRepo->findAllActive();
        $campaignItemsList = $client->campana_id ? $campaignRepo->findItemsByCampaignId($client->campana_id) : array();
        ...
        $this->render('client_form', array(
            ...
            'campaignsList' => $campaignsList,
            'campaignItemsList' => $campaignItemsList,
            'errors' => array()
        ));
    }
    ...
    public function save() {
        ...
        $campana_id = isset($_POST['campana_id']) && $_POST['campana_id'] !== '' ? (int)$_POST['campana_id'] : null;
        $campana_item_id = isset($_POST['campana_item_id']) && $_POST['campana_item_id'] !== '' ? (int)$_POST['campana_item_id'] : null;
        $cantidad_items = isset($_POST['cantidad_items']) ? (int)$_POST['cantidad_items'] : 1;
        if ($cantidad_items < 1) $cantidad_items = 1;
        ...
        if ($campana_item_id) {
            $campaignRepo = new MysqlCampaignRepository();
            $item = $campaignRepo->findItemById($campana_item_id);
            if ($item) {
                $comision_aplicada = $item->comision_venta * $cantidad_items;
                $precio_aplicado = $item->precio * $cantidad_items;

                if (strtolower($estado_llamada) === 'exito pedido pendiente') {
                    if (!empty($id)) {
                        $existing = $this->getClientByIdUseCase->execute($id);
                        if ($existing && strtolower($existing->estado_llamada) === 'exito pedido pendiente') {
        ...
        if ($id > 0) {
            try {
                ...
                if ($client) {
                    ...
                    if ($client->campana_item_id) {
                        $campaignRepo = new MysqlCampaignRepository();
                        $item = $campaignRepo->findItemById($client->campana_item_id);
                        if ($item) {
                            $qty = $client->cantidad_items > 0 ? $client->cantidad_items : 1;
                            $comision_aplicada = $item->comision_venta * $qty;
                            $precio_aplicado = $item->precio * $qty;
                    ...
                    $sql = "UPDATE `biartet_clientes` SET 
                        `estado_llamada` = 'Exito pedido pendiente', 
